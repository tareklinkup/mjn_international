...
class Check extends CI_Controller
{
	...
	public function check_list()
	{
		...
		$data['title'] = 'All Cheque Information';
		$data['checks'] = $this->Check_model->get_all_check_info();
...
//			print_r($this->session->userdata("cheque"));
	...
	public function getCustomerCheques(){
        $data = json_decode($this->input->raw_input_stream);

        $clauses = "";
        if(isset($data->dateFrom) && $data->dateFrom != '' && isset($data->dateTo) && $data->dateTo != ''){
            $clauses .= " and cc.csubmit_date between '$data->dateFrom' and '$data->dateTo'";
        }

        if(isset($data->customerId) && $data->customerId != '' && $data->customerId != null){
            $clauses .= " and cc.cus_id = '$data->customerId'";
        }

        $checques = $this->db->query("
            select
			cc.*,
                c.Customer_Code,
                c.Customer_Name,
                c.Customer_Mobile
            from tbl_checks cc
            join tbl_customer c on c.Customer_SlNo = cc.cus_id
            where cc.status = 'a'
            and cc.branch_id = ? 
			$clauses
            order by cc.id desc
