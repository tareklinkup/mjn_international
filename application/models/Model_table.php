...
class Model_Table extends CI_Model{
    ...
    public function getTransactionSummary($date = null){
        $transactionSummary = $this->db->query("
            ...
            (
                select ifnull(sum(bc.check_amount), 0) from tbl_checks bc
                where bc.check_status = 'Pa'
                and bc.status = 'a'
                and bc.branch_id= " . $this->session->userdata('BRANCHid') . "
                " . ($date == null ? "" : " and bc.csubmit_date < '$date'") . "
            ) as received_customer_cheque,
            (
            ...
            /* total */
            (
                select received_sales + received_customer + received_customer_cheque + received_supplier + received_cash + bank_withdraw + loan_received + loan_initial_balance + invest_received + sale_asset
            ) as total_in,
            (
    ...
    public function customerDue($clauses = "" , $date = null) {
        ...
        $dueResult = $this->db->query("
            ...
            (select ifnull(sum(bc.check_amount), 0.00) 
                from tbl_checks bc 
                where bc.cus_id = c.Customer_SlNo 
                and bc.check_status = 'Pa'
                " . ($date == null ? "" : " and bc.csubmit_date < '$date'") . "
                and bc.status = 'a') as chequepaidAmount,
            ...
            (select invoicePaid + cashReceived + chequepaidAmount) as paidAmount,
    ...
    public function getCustomerDueById($Custid)
    {
        ...
        // ====================
        $chequePaidAmount = $this->db->where('cus_id', $Custid)->where(['check_status'=> 'Pa', 'status'=>'a'])->select_sum('check_amount')->get('tbl_checks')->row();
        $chpaidAm = $chequePaidAmount->check_amount;
        ...
        $due = ($dueAm + $payAm + $prevDue) - ($paidAm + $chpaidAm + $salesReturned);
